/*
 * WPM - Wilix Package Manager (wget with --no-check-certificate)
 * Сборка: musl-gcc -static -O2 -std=c99 -D_DEFAULT_SOURCE -o wpm wpm.c
 * Зависимости: wget
 */
...
/* ---------- конфигурация ---------- */
#define BASE_URL    "https://deftly-masculine-possum.cloudpub.ru"
...
/* ---------- загрузка с прогресс-баром (wget с --no-check-certificate) ---------- */
static int download_with_progress(const char *url, const char *output, size_t expected_size) {
    printf("\n");
    
    // Запускаем wget с --no-check-certificate
    pid_t pid = fork();
    if (pid == 0) {
